...
export const Transfer = () => {
  const { publicKey, requestRecords, requestRecordPlaintexts, decrypt, transactionStatus } = useWallet()
  ...
  const handleSubmit = async (event: FormEvent, fnName: string) => {
    if (fnName === 'transfer_public_to_private') {
  ...
  const handleAmountChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const suitableRecord = getSuitableUnspentRecord(plainRecords, Number(event.currentTarget.value))
    ...
    >
      ...
      <button
        ...
        onClick={async (event: FormEvent) => {
          await handleSubmit(event, 'transfer_public_to_private')
        }}
      >
        Public to private
      </button>
      <button
        ...
        type="submit"
        onClick={async (event: FormEvent) => {
          await handleSubmit(event, 'transfer_private_to_public')
        }}
      >
        Private to public
      </button>
